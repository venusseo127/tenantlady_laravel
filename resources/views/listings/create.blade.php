<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-slate-800 leading-tight">Add Listing</h2></x-slot>
                ...
                <form method="POST" action="{{ route('listings.store') }}">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:ring-rose-500 focus:border-rose-500">{{ old('description') }}</textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700">Price (₱/month)</label>
                            <input type="number" name="price" value="{{ old('price', 0) }}" min="0" step="0.01" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:ring-rose-500 focus:border-rose-500">
                    ...
                    <div class="mt-6 flex gap-3">
                        <button type="submit" class="px-4 py-2 bg-rose-600 text-white rounded-lg font-medium hover:bg-rose-700">Create</button>
                        <a href="{{ route('listings.index') }}" class="px-4 py-2 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
